...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Evolutionary multiobjective optimization} \label{S_MEO}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{enumerate}
Note that this initialization scheme is not fully random as the initialization results in truck paths which are constructed by concatenating two shortest paths---the shortest path from the source node to the one randomly selected and the shortest path from the random node to the destination, which is added due to the representation described above.
...
- show that the algorithm works in principle on practical examples in manageable time
- check which of the two initialization schemes is 
In addition, a few more experiments show that the standard parameters of the algorithm behaves as with other test problems: population size, environmental selection (change from HypE to NSGA-II)

As a test case, we chose to use a real-world case study based on the work of \citet{bianco09}. The network used here consists of 311 nodes and 441 arcs and represents the main transport roads in Italy's Lazio region which amongst other cities includes the capital Rome.

The risks $r_{ij}^c$ for each arc $(i,j) \in A$ and commodity $c \in C$ are evaluated as the societal risk computed as the number of people living inside the exposure zone around link $(i,j)$ (whose size depends on the hazmat type $c$ times the accident probability involving the vehicle), for details see  \citep{bianco09}.
...
population size: 100, 2000 generations, 10 runs, hypervolume's reference point for HypE as in Table~\ref{tab:referencepoints} chosen according to initial objective function values: such that all initial solutions have a contribution to the hypervolume indicator (ref point the same in all objectives due to off-the-shelf PISA implementation of HypE), hypervolume computation exact
...
\subsection{The Influence of Standard Parameters}
\begin{itemize}
	\item show exemplarily the influence of some of the standard parameters of the EMO algorithm which 
\end{itemize}
